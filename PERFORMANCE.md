...
The continuous dashboard pairs anatomical body and neural activity in equal-width canvases. The spike raster, neuron matrix and retina sit directly below; physiology, connectivity, populations, learning and lifetime records follow on the same page. Navigation scrolls to sections instead of hiding them. Offscreen instruments skip painting while the same neural loop keeps running. Materials use neutral lighting and slate surfaces; the flybody meshes and their anatomical colours are retained. The timings above precede the dashboard layout change.
